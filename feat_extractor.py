...
f_trans = open('./tmp/trans_matrix.csv', 'r')
for f in f_trans:
...
def main():
    ...
    for f in fi:
        ...
        # # advanced linguistic feature: pronouncable domain
        # gib_value = int(gib_detect_train.avg_transition_prob(main_domain.strip('$'), model_mat) > threshold)
        feature = extract_feature(domain)
        if not feature:
            continue
        feature.insert(1, cla)
        try:
            fw.write(
                '%s\t%s\t%s\t%.3f\t%.1f\t%.3f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%d\n'
                % tuple(feature)
            )
