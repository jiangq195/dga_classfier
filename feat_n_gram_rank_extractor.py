...
fi = open('./tmp/training_w_tld.txt', 'r')
fw = open('./tmp/gram_ranks_training.txt', 'w')
...
for f in fi:
    ...
    bigram_rank = [gram_rank_dict[''.join(i)] if ''.join(i) in gram_rank_dict else 0 for i in
                   bigrams(main_domain)]  # extract the bigram
    trigram_rank = [gram_rank_dict[''.join(i)] if ''.join(i) in gram_rank_dict else 0 for i in
