...
def get_feat_table():
    fi = open('./tmp/features_norm.txt', 'r')
...
def get_vec():
    vec = DictVectorizer()
    feat_table = get_feat_table()
    measurements = [feat_dict for domain, cla, feat_dict in feat_table]
    feature_list = vec.fit_transform(measurements).toarray()

    return feature_list, vec, feat_table


def write_vec_feature_w_ranks_norm():
    feature_list, vec, feat_table = get_vec()
    feature_header = vec.get_feature_names()

    fw_out = open('./tmp/vectorized_feature_w_ranks_norm.txt', 'w')
...
if __name__ == '__main__':
    write_vec_feature_w_ranks_norm()
